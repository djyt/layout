...
public:
    ...
    ORoad(QList<HeightSegment> *heightSections, HWRoad* hwroad, RomLoader *rom);
    ~ORoad();
    void init();
    void tick();
    int16_t get_road_y(uint16_t);
    void setHeightMap(int map_index, int height_index, int height_start, int height_end = -1);
    void setDelay(int height_delay);
    void setHorizonMod(int horizon_mod);
    void setHorizon(int horizon_base);
    void removeUserOffset(int userY);

private:
    QList<HeightSegment>* heightSections;
    HeightSegment heightSeg;
    HWRoad* hwroad;
    RomLoader* rom;
