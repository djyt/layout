...
ORoad::ORoad(QList<HeightSegment>* heightSections, HWRoad* hwroad, RomLoader* rom)
{
...
{
    const uint32_t len = levelData->end_pos - 1;
    QPoint p1 = levelData->path[addr + 0 <= len ? addr + 0 : len];
    QPoint p2 = levelData->path[addr + 1 <= len ? addr + 1 : len];

    const int16_t x = p1.x() + p2.x(); // Length 1
    const int16_t y = p1.y() + p2.y(); // Length 2

    // Use Pythagorus' theorem to find the distance/length between x & y
    const uint16_t distance = isqrt((x * x) + (y * y));
    ...
    {
        p1 = levelData->path[addr <= len ? addr++ : len];
        p2 = levelData->path[addr <= len ? addr++ : len];

        const int32_t x_next = p1.x() + p2.x(); // Length 1
        const int32_t y_next = p1.y() + p2.y(); // Length 2
...
void ORoad::set_tilemap_x(uint32_t addr)
{
    ...
    const uint32_t len = levelData->end_pos - 1;
    QPoint p1 = levelData->path[addr <= len ? addr++ : len];
    QPoint p2 = levelData->path[addr <= len ? addr++ : len];
    QPoint p3 = levelData->path[addr <= len ? addr++ : len];
    QPoint p4 = levelData->path[addr <= len ? addr++ : len];
...
void ORoad::setHeightMap(int map_index, int height_index, int height_start, int height_end)
{
    // Get Address of actual road height data
    height_lookup_wrk = height_lookup = map_index;
    heightSeg         = heightSections->at(height_lookup);
    height_ctrl2      = heightSeg.type;
    step_adjust       = heightSeg.step;

    this->height_index = height_index;
    this->height_start = height_start;
    this->height_end   = height_end != -1 ? height_end : height_start;
}

void ORoad::setDelay(int height_delay)
{
    this->height_delay = height_delay;
    elevation          = FLAT; // Keep horizon at level for delayed section
}

void ORoad::setHorizonMod(int horizon_mod)
{
    this->horizon_mod = horizon_mod - horizon_base;
    horizon_set = 1;
}

void ORoad::setHorizon(int horizon_base)
{
    this->horizon_base = horizon_base;
    horizon_set = 1;
}

void ORoad::removeUserOffset(int userY)
{
    this->horizon_base -= userY;
}
...
void ORoad::init_height_seg()
{
    ...
    uint32_t h_addr = 0; // Hard Coded For Layout
    height_ctrl2   = heightSeg.type;
    step_adjust    = heightSeg.step;
...
{
    down_mult   = heightSeg.value1;
    up_mult     = heightSeg.value2;
    height_addr = addr;
...
void ORoad::init_horizon_adjust(uint32_t& addr)
{
    ...
    horizon_mod = heightSeg.value1 - horizon_base;
...
void ORoad::set_y_interpolate()
{
    ...
    a1_lookup = height_index + height_addr;
    ...
    const int16_t next_height_value = heightSeg.data.at(a1_lookup);
...
void ORoad::set_y_2044()
{
    ...
    if ((int) a1_lookup < heightSeg.data.size() - 1)
        return;
...
void ORoad::read_next_height()
{
    ...
    change_per_entry = heightSeg.data.at(a1_lookup++) << 4;
...
void ORoad::do_road_data()
{
    ...
    int16_t scanline = 255; //  Loop counter #2 (Scanlines) - Start at bottom of screen
...
void ORoad::copy_bg_color()
{
    ...
    for (int i = 0; i < 32; i++)
    {
        hwroad->write32(&dst, rom->read32(&src));
        hwroad->write32(&dst, rom->read32(&src));
        hwroad->write32(&dst, rom->read32(&src));
        hwroad->write32(&dst, rom->read32(&src));
        hwroad->write32(&dst, rom->read32(&src));
...
int32_t ORoad::next(const int32_t n, const int32_t i)
{
    return (n + i/n) >> 1;
}

int32_t ORoad::isqrt(const int32_t number)
{
    if (number == 0)
        return 0;
