...
// Screen X, Y Co-ordinates
int32_t osprite::get_screen_x1()
{
    return data[0x6] - 0xBE;
}

// This value is only known after the render has been called!
int32_t osprite::get_screen_x2()
{
    return get_screen_x1() + get_x_delta() * get_screen_width();
}

int32_t osprite::get_screen_y1()
{
    return (data[0] & 0x1ff) - 0x100;
}

int32_t osprite::get_screen_y2()
{
    return get_screen_y1() + get_y_delta() * get_screen_height();
}

// This value is only known after the render has been called!
int32_t osprite::get_screen_width()
{
    return width;
}

int32_t osprite::get_screen_height()
{
    return (data[5] >> 8) + 1;
}

int32_t osprite::get_x_delta()
{
    return ((data[4] & 0x2000) != 0) ? 1 : -1;
}

int32_t osprite::get_y_delta()
{
    return ((data[4] & 0x8000) != 0) ? 1 : -1;
}

bool osprite::is_hidden()
{
    return (data[0] & 0x5000) || get_screen_height() == 0;
}
